...
def test_create_user_new_email(
    ...
    with (
        patch(
            "specifai.general.backend.utils.utils.send_email",
            return_value=None,
        ),
        patch(
            "specifai.general.backend.components.config.settings.SMTP_HOST",
            "smtp.example.com",
        ),
        patch(
            "specifai.general.backend.components.config.settings.SMTP_USER",
            "admin@example.com",
        ),
    ):
...
def test_get_existing_user(
    ...
    user = create_user_with_workspace(db, user_in)
    user_id = user.id
    r = client.get(
...
def test_create_user_existing_username(
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    create_user_with_workspace(db, user_in)
    data = {"email": username, "password": password}
    r = client.post(
        f"{settings.API_V1_STR}/users/",
        headers=superuser_token_headers,
...
def test_retrieve_users(
    ...
    create_user_with_workspace(db, user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = create_user_with_workspace(db, user_in2)
